...
void max_degree(Node *part, int size);                               // finds the node with maximum degree 
...
/* finds the node with maximum degree */
void max_degree(Node *part, int size)
{
	int i, max_degree, max_node = 0;
		
	max_degree = part[0].degree;
	for(i = 0; i < size; i++)
	{
		if(max_degree < part[i].degree)
		{
			max_degree = part[i].degree;
			max_node = i;
		}
	}
	
	swap(part, max_node, size - 1); // put the node with the maximum degree to the end of the partition
}
...
int find_k(Node *left, Node *right, int **sq, int lsize, int rsize)
{
	...
	while(lsize != 0 && rsize != 0)        // U and V != 0
	{
		...
		{
			max_degree(left, lsize);  // node in left partition with maximum degree
		...
		{
			max_degree(right, rsize); // node in right partition with maximum degree
			...
			rsize -= 1; // decreases the size of right partition
